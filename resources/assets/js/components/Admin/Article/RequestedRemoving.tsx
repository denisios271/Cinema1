...
export default class RequestedRemoving extends React.Component {
    render() {
        return <RequestedLayer prefix="article" createElement={(v: iArticle, events: iEvents) => (
            <div className="card mb-3" key={v.id}>
                <div className="card-body">
                    ...
                    <a href="" onClick={e => { e.preventDefault(); events.acceptRemoving(v.id) }} className="btn btn-success mr-3">Подтвердить!</a>
                    <a href="" onClick={e => { e.preventDefault(); events.declineRemoving(v.id) }} className="btn btn-danger">Отменить!</a>
                </div>
            </div>
        )} send = {() => Api.get(`article/requested/removing`)} />
    }
}